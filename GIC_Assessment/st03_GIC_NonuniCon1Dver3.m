...
function [sumGIVNS,sumGIVEW] = st03_GIC_NonuniCon1Dver3(pointerQ,GIC_GlobalSettings,GIC_Calculation,GIC_FieldData,LineCorridorData)
...
if GIC_GlobalSettings.EFieldConductivityZone==0  % Use the EPRI 1D model
    ...
    if LineCorridorData
        Line_Lat = LineCorridorData(:,1)';
        Line_Lon = LineCorridorData(:,2)';
    else
        Line_Lat = [FromLat:round((ToLat-FromLat)/Branch_updatefix(pointerQ,7)*Calc_accuracy,10):ToLat];
        Line_Lon = [FromLon:round((ToLon-FromLon)/Branch_updatefix(pointerQ,7)*Calc_accuracy,10):ToLon];
    end
